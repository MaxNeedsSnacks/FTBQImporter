...
/*
 * This class has been taken (with explicit permission) from Funwayguy's Better
 * Questing mod. Original package: betterquesting.api.utils
 */
public class NBTConverter {
    ...
    if (tag instanceof NBTTagString) {
      ...
      if (format) {
        ...
        for (int i = 0; i < tagList.tagCount(); i++) {
          jAry.add(i + ":" + tagList.get(i).getId(),
      ...
      return new JsonObject(); // No valid types found. We'll just return this
                               // to prevent a NPE
    }
  }
  ...
  public static long[] readLongArray(NBTTagLongArray tag) {
    ...
    String[] entry = tag.toString()
                         .replaceAll("[\\[\\]L;]", "")
                         .split(","); // Cut off square braces and "L;" before
                                      // splitting elements
  ...
  public static JsonObject NBTtoJSON_Compound(NBTTagCompound parent,
                                              JsonObject jObj, boolean format) {
    if (parent == null) {
      return jObj;
    }
  ...
  JSONtoNBT_Object(JsonObject jObj, NBTTagCompound tags, boolean format) {
    if (jObj == null) {
      return tags;
    }
    ...
    for (Entry<String, JsonElement> entry : jObj.entrySet()) {
      ...
      if (!format) {
        ...
        try {
          ...
          if (tags.hasKey(key)) {
            FTBQImporter.LOGGER.log(Level.WARN,
                                    "JSON/NBT formatting conflict on key '" +
                                        key + "'. Skipping...");
            continue;
          }
        }
  ...
  private static NBTBase JSONtoNBT_Element(JsonElement jObj, byte id,
    ...
    try {
        ...
        if (jObj.isJsonArray()) {
          ...
          for (Entry<String, JsonElement> entry : jAry.entrySet()) {
            try {
              ...
              // String key = entry.getKey().substring(0,
              // entry.getKey().lastIndexOf(":" + id));
              tList.appendTag(JSONtoNBT_Element(entry.getValue(), id2, format));
            } catch (Exception e) {
              tList.appendTag(
                  JSONtoNBT_Element(entry.getValue(), (byte)0, format));
            }
          }
        }
      ...
      FTBQImporter.LOGGER.log(
          Level.ERROR,
          "An error occured while parsing JsonElement to NBTBase (" + tagID +
              "):",
          e);
    }

    FTBQImporter.LOGGER.log(Level.WARN, "Unknown NBT representation for " +
                                            jObj.toString() + " (ID: " + tagID +
                                            ")");
    return new NBTTagString();
  }
    ...
    if (jObj.isJsonPrimitive()) {
      ...
      if (prim.isNumber()) {
        if (prim.getAsString().contains(
                ".")) // Just in case we'll choose the largest possible
                      // container supporting this number type (Long or Double)
        {
          tagID = 6;
        } else {
          tagID = 4;
      ...
      for (JsonElement entry : array) {
        if (entry.isJsonPrimitive() &&
            tagID == 0) // Note: TagLists can only support Integers, Bytes and
                        ...
                        // handling)
        {
          try {
            for (JsonElement element : array) {
              ...
              if (element.getAsLong() !=
                  element.getAsByte()) // In case casting works but overflows
              {
            ...
            try {
              for (JsonElement element : array) {
                ...
                if (element.getAsLong() !=
                    element.getAsInt()) // In case casting works but overflows
                {
                  throw new ClassCastException();
                }
              }
