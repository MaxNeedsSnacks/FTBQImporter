...
public class CommandImport extends CommandBase {
  ...
  public void importQuests(MinecraftServer server, ICommandSender sender,
                           List<String> flags) throws CommandException {

    if (!flags.isEmpty()) {
      sender.sendMessage(new TextComponentString(
          "Flags supplied: " + joinNiceStringFromCollection(flags)));
    }

    // additional flags that may be supplied
    boolean fix_icons = flags.contains("-i") || flags.contains("--fix-icons");
    boolean truncate_loot =
        flags.contains("-l") || flags.contains("--truncate-loot");
    boolean auto_cmd = flags.contains("-c") || flags.contains("--auto-cmd");

    JsonElement defaultQuestsJson =
        DataReader
            .get(new File(Loader.instance().getConfigDir(),
                          "betterquesting/DefaultQuests.json"))
            .safeJson();
    JsonElement defaultLootJson =
        DataReader
            .get(new File(Loader.instance().getConfigDir(),
    ...
    if (!defaultQuestsJson.isJsonObject()) {
      sender.sendMessage(new TextComponentString(
          "config/betterquesting/DefaultQuests.json not found!"));
      return;
    }

    NBTTagCompound defaultQuests = NBTConverter.JSONtoNBT_Object(
        defaultQuestsJson.getAsJsonObject(), new NBTTagCompound(), true);

    String ver;
    if (!(ver = defaultQuests.getString("format")).startsWith("2")) {
      sender.sendMessage(new TextComponentString(
          "Cannot import DefaultQuests.json with old version (" + ver + ")!"));
      return;
    }
    ...
    FileUtils.deleteSafe(new File(f.universe.getWorldDirectory(),
                                  "data/ftb_lib/teams/ftbquests"));
    ...
    if (defaultLootJson.isJsonObject()) {
      NBTTagCompound defaultLoot = NBTConverter.JSONtoNBT_Object(
          defaultLootJson.getAsJsonObject(), new NBTTagCompound(), true);
      ...
      for (NBTBase groupBase : defaultLoot.getTagList("groups", 10)) {
        ...
        table.lootCrate.stringID =
            StringUtils.getID(table.title, StringUtils.FLAG_ID_DEFAULTS);
        table.lootCrate.itemName = table.title;

        crateTable.rewards.add(new WeightedReward(
            new ItemReward(crateTable.fakeQuest, table.lootCrate.createStack()),
            groupNbt.getInteger("weight")));
        ...
        for (NBTBase rewardBase : defaultLoot.getTagList("rewards", 10)) {
          ...
          for (NBTBase rewardItem : rewardNbt.getTagList("items", 10)) {
            ...
            if (!stack.isEmpty()) {
              table.rewards.add(
                  new WeightedReward(new ItemReward(table.fakeQuest, stack),
                                     rewardNbt.getInteger("weight")));
              break;
            }
          }
        }
    ...
    for (NBTBase questBase : defaultQuests.getTagList("questDatabase", 10)) {
      ...
      NBTTagCompound properties = questNbt.getCompoundTag("properties")
                                      .getCompoundTag("betterquesting");
      ...
      quest.taskLogicAnd =
          properties.getString("tasklogic").equalsIgnoreCase("AND");
      ...
      for (NBTBase taskBase : questNbt.getTagList("tasks", 10)) {
        ...
        case "bq_standard:retrieval": {
          BQItemTask task =
              quest.taskLogicAnd ? new BQItemAndTask() : new BQItemOrTask();
        ...
        default:
          sender.sendMessage(new TextComponentString(
              "Can't import task with type " + type +
              ", you will have to manually re-add it!"));
          break;
        }
      }
      // endregion tasks
      ...
      for (NBTBase rewardBase : questNbt.getTagList("rewards", 10)) {
        ...
        case "bq_standard:item": {
          for (NBTBase rewardItem : rewardNbt.getTagList("rewards", 10)) {
            ...
            if (truncate_loot &&
                ItemStack.areItemStackTagsEqual(reward.item, loot_chest) &&
                crateTable != null) {
        ...
        case "bq_standard:command": {
          ...
          reward.command =
              rewardNbt.getString("command").replace("VAR_NAME", "@p");
        ...
        default:
          sender.sendMessage(
              new TextComponentString("Can't import reward with type " + type));
          break;
        }
      }
      // endregion rewards
    ...
    for (NBTBase chapterBase : defaultQuests.getTagList("questLines", 10)) {
      ...
      NBTTagCompound properties = chapterNbt.getCompoundTag("properties")
                                      .getCompoundTag("betterquesting");
      ...
      for (NBTBase questBase : chapterNbt.getTagList("quests", 10)) {
        ...
        if (quest != null && !mappedQuests.contains(quest)) {
          double sizeX =
              MathHelper.clamp(Math.max(questNbt.getDouble("size"),
                                        questNbt.getDouble("sizeX")) /
                                   24D,
                               0.5, 3);
          double sizeY =
              MathHelper.clamp(Math.max(questNbt.getDouble("size"),
                                        questNbt.getDouble("sizeY")) /
                                   24D,
                               0.5, 3);
    ...
    int table_size =
        (int)Math.ceil(MathUtils.sqrt(orphanChapter.quests.size()));
    ...
    for (BQChapter chapter : chapters) {
      ...
      for (BQQuest quest : chapter.quests) {
        ...
        for (BQReward reward : quest.rewards) {
          ...
          r.autoclaim = (reward instanceof BQCommandReward && auto_cmd)
                            ? RewardAutoClaim.INVISIBLE
                            : quest.autoClaim ? RewardAutoClaim.ENABLED
                                              : RewardAutoClaim.DEFAULT;
    ...
    newQuestMap.forEach((bqQuest, ftbQuest) -> {
      ...
      questTaskMap.get(ftbQuest).forEach((bqTask, ftbTasks) -> {
        taskInfo.setIntArray(
            bqTask.toString(),
            Arrays.stream(ftbTasks).mapToInt(task -> task.id).toArray());
      });
    });
    ...
    NBTUtils.writeNBTSafe(
        new File(Loader.instance().getConfigDir(), "imported_quests.nbt"),
        exportedData);

    sender.sendMessage(
        new TextComponentString("Finished importing Quests and Loot!"));
    server.getPlayerList().sendMessage(new TextComponentString(
        "Server has successfully imported quests and loot tables from Better Questing! Rejoin the world or server now to get the updated quests."));
    server.getPlayerList().sendMessage(new TextComponentString(
        "Make sure to double-check everything as well, as the two mods are fundamentally different from one another."));
  }

  public void importProgress(MinecraftServer server, ICommandSender sender,
                             List<String> flags) throws CommandException {
    final Universe u = Universe.get();

    JsonElement questingPartiesJson =
        DataReader
            .get(new File(u.getWorldDirectory(),
                          "betterquesting/QuestingParties.json"))
            .safeJson();
    if (!questingPartiesJson.isJsonObject()) {
      sender.sendMessage(new TextComponentString(
          "betterquesting/QuestingParties.json was not found in your save!"));
      return;
    }
    NBTTagCompound questingParties = NBTConverter.JSONtoNBT_Object(
        questingPartiesJson.getAsJsonObject(), new NBTTagCompound(), true);

    JsonElement nameCacheJson =
        DataReader
            .get(new File(u.getWorldDirectory(),
                          "betterquesting/NameCache.json"))
            .safeJson();
    if (!nameCacheJson.isJsonObject()) {
      sender.sendMessage(new TextComponentString(
          "betterquesting/NameCache.json was not found in your save!"));
      return;
    }
    NBTTagCompound nameCache = NBTConverter.JSONtoNBT_Object(
        nameCacheJson.getAsJsonObject(), new NBTTagCompound(), true);
    ...
    for (NBTBase partyBase : questingParties.getTagList("parties", 10)) {
      ...
      party.name = partyNbt.getCompoundTag("properties")
                       .getCompoundTag("betterquesting")
                       .getString("name")
                       .trim();
      ...
      for (NBTBase memberBase : partyNbt.getTagList("members", 10)) {
        ...
        if (p == null || status.equals("INVITE"))
          continue;
    ...
    for (BQParty party : parties) {
      ...
      if (!owner.hasTeam()) {
        ...
        String team_id_base =
            StringUtils.getID(owner.getName(), StringUtils.FLAG_ID_DEFAULTS);
        ...
        u.clearCache();
        team = new ForgeTeam(u, u.generateTeamUID((short)0), team_id,
                             TeamType.PLAYER);
        team.setColor(
            EnumTeamColor.NAME_MAP.getRandom(sender.getEntityWorld().rand));
        owner.team = team;
        team.owner = owner;
        ...
        if (owner.isOnline()) {
          owner.getPlayer().sendMessage(new TextComponentString(
              "Quest data for your party is being synced with your new team " +
              team.getId()));
        }
        ...
        if (owner.isOnline()) {
          owner.getPlayer().sendMessage(new TextComponentString(
              "Quest data for your party is being synced with your current team " +
              team.getId()));
        }
      }
      ...
      for (ForgePlayer player : party.members) {
        if (!player.equalsPlayer(owner) && player.isOnline()) {
          player.getPlayer().sendMessage(new TextComponentString(
              "Quest data is being synced with your party owner's team " +
              team.getId() +
              ". Please join this team if you wish to keep your progress"));
        }
    ...
    if (!questProgressJson.isJsonObject()) {
      sender.sendMessage(new TextComponentString(
          "betterquesting/QuestProgress.json was not found in your save!"));
      return;
    }
    NBTTagCompound questProgress = NBTConverter.JSONtoNBT_Object(
        questProgressJson.getAsJsonObject(), new NBTTagCompound(), true);

    // FIXME: Don't load JSON, instead use NBT for consistency purposes
    NBTTagCompound conversionMapping = NBTUtils.readNBT(
        new File(Loader.instance().getConfigDir(), "imported_quests.nbt"));

    if (conversionMapping == null) {
      sender.sendMessage(new TextComponentString(
          "No valid conversion mapping was found in your config folder!"));
      return;
    }

    if (conversionMapping.getInteger("_version") != CONVERT_FILE_VERSION) {
      sender.sendMessage(new TextComponentString(
          "The conversion mapping found is using a different format. Please run quest importing again!"));
      return;
    }
    ...
    for (NBTBase questProgressBase :
         questProgress.getTagList("questProgress", 10)) {
      ...
      for (NBTBase completedBase :
           questProgressNbt.getTagList("completed", 10)) {
      ...
      for (NBTBase taskDataBase : questProgressNbt.getTagList("tasks", 10)) {
        ...
        for (NBTBase completeUser :
             taskDataNbt.getTagList("completeUsers", 8)) {
        ...
        questData.completed.forEach((forgePlayer, claimed) -> {
          ...
          if (claimed) {
            quest.rewards.forEach(
                reward
                -> teamData.setRewardClaimed(forgePlayer.getId(), reward));
          }
        });

        questData.tasks.forEach(taskData -> {
          getConvertedTasks(f, questData.id, taskData.id, conversionData)
              .forEach(opt -> opt.ifPresent(task -> {
                ...
                taskData.completeUsers.forEach(forgePlayer -> {
                  ...
                  if (!(processed.contains(team) ||
                        processedTask.contains(team))) {
                    task.forceProgress(teamData, ChangeProgress.COMPLETE_DEPS,
                                       false);
    ...
    server.getPlayerList().sendMessage(new TextComponentString(
        "Server has successfully migrated quest progression for all Better Questing parties to FTB Quests!"));
    server.getPlayerList().sendMessage(new TextComponentString(
        "Players, please note that you may be required to join your former party owner's team in order to get your progress back!"));
  }
  ...
  private ItemStack nbtItem(NBTTagCompound itemNbt, boolean isFilter) {
    ...
    if (itemNbt.isEmpty()) {
      FTBQImporter.LOGGER.debug(
          "Item Task {} has incorrect format! Returning empty item stack",
          itemNbt);
      return stack;
    }
    ...
    if (id.isEmpty() || id.equals("betterquesting:placeholder")) {
      FTBQImporter.LOGGER.debug(
          "Item ID {} is invalid or empty! Returning empty item stack", id);
      return stack;
    ...
    if (stack.isEmpty() ||
        stack.isItemEqual(new ItemStack(ItemFiltersItems.MISSING))) {
      FTBQImporter.LOGGER.debug("{} returned an empty or missing item!",
                                itemNbt);
    } else {
    ...
    if (isFilter) {
      ...
      if (filter instanceof ItemFilter.ItemFilterData) {
        ...
        if (valid_items.isEmpty()) {
          FTBQImporter.LOGGER.debug(
              "Warning: Did not create Ore Dictionary filter with value {} as it is empty!",
              ore);
          return ItemStack.EMPTY;
        }

        FTBQImporter.LOGGER.debug(
            "Successfully created ore dictionary filter with value {}", ore);
        return oreFilter;
      }
  ...
  private Optional<Quest> getConvertedQuest(ServerQuestFile f, int qid,
                                            NBTTagCompound mapping) {
    return Optional.ofNullable(f.getQuest(
        mapping.getCompoundTag(Integer.toString(qid)).getInteger("id")));
  }

  private List<Optional<Task>> getConvertedTasks(ServerQuestFile f, int qid,
                                                 int tid,
                                                 NBTTagCompound mapping) {
  ...
  private static abstract class BQReward { abstract Reward create(Quest q); }
  ...
  private static class BQChoiceReward extends BQReward {
    ...
    Reward create(Quest q) {
      ...
      for (ItemStack stack : items) {
        reward.table.rewards.add(new WeightedReward(
            new ItemReward(reward.table.fakeQuest, stack), 1));
      }
